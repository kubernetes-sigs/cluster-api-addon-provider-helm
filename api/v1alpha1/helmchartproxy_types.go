...
type HelmChartProxySpec struct {
	...
	// and be kept up to date with the latest version. If VersionMap is set, this field must be empty.
	// +optional
	...
	// If a Cluster's Kubernetes version matches multiple ranges or exact versions, the first match will be used based on the sort order
	// of the map keys.
	//
	...
	// Note: exact versions with patch versions must be specified for OCI based Helm charts.
